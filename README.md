...
`-- scripts                                         # directory containing scripts to prepare and deploy infrastructure and app
    ...
    |-- deploy.sh                                   # bash script to deploy the infrastructure: k8s cluster, prometheus, grafana, spinnaker, minio
    `-- create_spinnaker_app.sh                     # bash script to create a spinnaker demo app, canary config and pipelines             
```
...
3. Analyze the amount of 500 http_code requests between baseline and canary versions, go back to the running pipeline and on the _**Manual Judgment**_ stage click _**Stop**_ if you want to cancel the deployment or _**Continue**_ if you are satisfied with the metrics.

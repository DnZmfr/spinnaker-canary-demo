...
spin application save --file ../spinnaker/app-demo.json
sleep 5

bold "Create canary config"
spin canary canary-config save -f ../spinnaker/config-canary.json
...
sed -i.orig "s/SIMPLE_DEPLOY_PIPELINE_ID/$SIMPLE_DEPLOY_PIPELINE_ID/g" ../spinnaker/pipeline-automated-canary.json
...
mv ../spinnaker/pipeline-automated-canary.json.orig ../spinnaker/pipeline-automated-canary.json
